...
Agda y como estas nos permite expresar propiedades y diferentes tipos.
\end{block}
...
\item Entender porque \textit{funciona} el \cf{head} presentado anteriormente.\\
\item Presentar una implementación mas adecuada utilizando vectores.\\
...
Podemos ver que tenemos definido ya los \ct{$\N$} y las \ct{List} A, solo 
nos hace falta implementar el tipo $<$. Es interesante mencionar que para
...
información acerca del tipo de los elementos, si no además de su
tamaño.\\
...
Podemos pensar ahora como utilizar el tipo vector para realizar una
...
\begin{block}{}
¿Donde quedo el caso de pattern matching para la lista vacia, \cc{$[]$}?;
Para resolver esta pregunta, prestemos atención al tipo de este constructor.
\end{block}
...
\begin{block}{}
Ahora bien, considerando que puede ser un problema obtener el tamaño
del vector resultante, tal vez exista un tipo tal que podamos \textit{calcularlo}
durante el filtrado.
\end{block}
